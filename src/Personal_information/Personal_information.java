...
public class Personal_information extends javax.swing.JDialog {

    Connection connection;
    Personal_informationModel model;
    SizeSetter columnWidth;
    
    public Personal_information(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;

        model = new Personal_informationModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        NewPersonal_information np = new NewPersonal_information((Frame) this.getParent(), true, connection);
        np.setVisible(true);
        try {
            model.updateData();
        } catch (SQLException ex) {
            Logger.getLogger(Personal_information.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            NewPersonal_information np = new NewPersonal_information((Frame) this.getParent(), true, 
    ...
    private void jButtonDELETEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDELETEActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            model.delete(model.getSelectesItem(jTable1.getSelectedRow()).getidperson());
        }
        try {
            model.updateData();
        } catch (SQLException ex) {
