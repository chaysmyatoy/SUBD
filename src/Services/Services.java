...
public class Services extends javax.swing.JDialog {
    ...
    public Services(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;

        model = new ServicesModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        NewServices np = new NewServices((Frame) this.getParent(), true, connection);
        np.setVisible(true);
        try {
            model.updateData();
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            NewServices np = new NewServices((Frame) this.getParent(), true, 
    ...
    private void jButtonDELETEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDELETEActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            model.delete(model.getSelectesItem(jTable1.getSelectedRow()).getidservices());
        }
        try {
            model.updateData();
        } catch (SQLException ex) {
